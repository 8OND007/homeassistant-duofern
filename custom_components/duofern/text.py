...
Note: the 7 Umweltsensor trigger-threshold text entities that used to live
here (space-separated 5-value strings, e.g. "off 15 off off off") have been
replaced by the structured Grenzwert GUI in number.py/switch.py/select.py —
one Number/Switch per field, reading/writing whichever of the 5 Grenzwerte
is currently selected via a per-group Select entity, instead of one text
field encoding all 5 at once. See coordinator.py's "Structured per-Grenzwert
GUI" section for the underlying data model.
"""
