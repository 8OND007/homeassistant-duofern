...
- **Fixed: several new Umweltsensor entities showed only the device name ("Wetterstation") with no distinguishing label** — the three Active-Grenzwerte sensors and the Dawn/Dusk event entity had `translation_key` set but no matching `strings.json`/`en.json`/`de.json` entry and no `_attr_name` fallback; with `has_entity_name=True` and no resolvable name, Home Assistant falls back to showing just the device name. Added both the missing translations and explicit English fallback names ("Sun Trigger Slots", "Wind Trigger Slots", "Temperature Trigger Slots", "Dawn/Dusk").
